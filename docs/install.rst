...
Important thing to remember before you run an update. Keep a copy of all the ".dotfiles", since the update will overwrite them. If you are running in a subdomain you have to reenable the "  RewriteRule ^(.*)$ public/$1 [L]"
Also if your installation have custom configuration in ".htacess" you have to reapply it.
